...
var getEqByNhc = function (id) {
  ...
  try {
    ...
    result = jLoc.find({name: {$regex: [tmpNL, 'i']}})
    for (var i = 0; i < result.length; i++) {
      if (result[i].name.length == tmpNL.length) {
        tmpJL = result[i].id
      }
      log.debug('getEqByNhc - found JLOC: ' + tmpJL)
    }
...
/*    var tmpJL = jLoc.findOne({
    ...
    result = jEq.find({name: {$regex: [tmpNE.name, 'i']}, location: tmpJL})
    for (var i = 0; i < result.length; i++) {
      if (result[i].name.length == tmpNE.name.length) {
        tmpJE = result[i]
      }
