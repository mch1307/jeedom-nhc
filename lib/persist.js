...
var initNhcEq = function (nhcEquData) {
  ...
  for (var i = 0; i < jsNhc.data.length; i++) {
    ...
    log.debug("initNhcEq inserted: "+ jsNhc.data[i])
  }
}
...
var getEqByNhc = function (id) {
  ...
  try {
    ...
    log.debug('getEqByNhc - found nhc: '+ tmpNE)
    var tmpNL = nLoc.findOne({
      'id': tmpNE.location
    }).name
    log.debug('getEqByNhc - found nhc loc: '+ tmpNL)
    ...
    log.debug('getEqByNhc - found Jequip: '+ tmpJE)
